...
for result_name, result_dataframe in divide_dataframe_regarding_mcc(pd.read_csv(results_file_path)).items():
    ...
    for mode in modes:
        ...
        for xai_name, xai_data in dataset_counts.items():
            for level_name, level_value in xai_data.items():
                x_values += [xai_name]
                y_values += [level_name]
        ...
        heatmap_data = df.pivot_table(index='XAIs', columns='Correlation Level', values='Value')
        ...
        plt.figure(figsize=(4, (2 + len(heatmap_data)) * 0.3))
        ...
        out_file_name = f"correlations_regarding_xai_{result_name}"
